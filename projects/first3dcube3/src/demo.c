...
void display(GLFWwindow *window, double time)
{
    ...
    for (int i = 0; i < 24; i++)
    {
        ...
        glm_translate(tMat, (vec3){sin(0.35f * (float) tf) * 8.0f, cos(0.52f * (float) tf) * 8.0f, sin(0.7f * (float) tf) * 8.0f});
        glm_rotate(rMat, 1.75f * (float) tf, (vec3){0.0f, 1.0f, 0.0f});
        glm_rotate(rMat, 1.75f * (float) tf, (vec3){1.0f, 0.0f, 0.0f});
        glm_rotate(rMat, 1.75f * (float) tf, (vec3){0.0f, 0.0f, 1.0f});
